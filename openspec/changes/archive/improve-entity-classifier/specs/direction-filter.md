...
**Status: ✅ COMPLETE** — all five FRs verified as implemented.

| Feature | Status | Verification |
|---------|--------|-------------|
| FR-1 Direction Filter | ✅ | `direction-filter.ts`, 196 tests, integrated in suggest-role + entity-enricher |
| FR-2 Rich AI Prompt | ✅ | money IN/OUT labels, samples, amounts in suggest-role prompt |
| FR-3 OTRO Persistence + Batch Review | ✅ | Schema, service, API, validation, EntityOnboardingModal, OTRO Review tab in EntityManagementPage with individual + batch reclassify |
| FR-4 Web Search Fallback | ✅ | `web-search-service.ts`, timeout, confidence cap, 8 unit tests |
| FR-5 Test Coverage | ✅ | 196+175+8+ existing tests |

### Closing Commits
- `da82dbe` — FR-3 UI (OTRO Review tab, batch reclassify) + FR-4 (8 web-search tests)
