...
describe('Atomicity — Transaction rollback on failure', () => {
  ...
  /**
   * C4/C5 atomicity: transaction rollback on real PostgreSQL constraint violation.
   *
   * Strategy: Use prisma.$transaction directly (same mechanism as
   * TransactionRunner). Perform two valid writes, then trigger a real
   * PostgreSQL unique constraint violation. The entire transaction
   * must roll back, leaving none of the writes persisted.
   *
   * This certifies that the TransactionRunner mechanism used by
   * repository.update() correctly rolls back all writes on failure.
   */
  it('C4/C5 atomicity: transaction rolls back all writes on constraint violation', async () => {
    // Step 1: Pre-create a MemoryItem AND its version 1 outside the transaction.
    // This ensures the unique constraint @@unique([itemId, versionNumber])
    // will trigger when the transaction attempts to create a duplicate.
    const refItem = await prisma.memoryItem.create({
      data: {
        content: 'Reference item for C4/C5 atomicity',
        type: 'fact',
        companyId: COMPANY_A,
        sourceAuthor: 'test',
        sourceName: 'test',
    ...
    await prisma.memoryVersion.create({
      data: {
        itemId: refItem.id,
        versionNumber: 1,
        content: 'Reference version 1',
        snapshot: {},
      },
    });

    // Step 2: Inside a transaction, perform two valid writes then trigger
    // a real constraint violation on @@unique([itemId, versionNumber])
    await expect(
      runTx(async (tx) => {
        // Write A: create a new MemoryItem — succeeds
        const newItem = await tx.memoryItem.create({
          data: {
            content: 'Write A — should not persist',
            type: 'fact',
            companyId: COMPANY_A,
            sourceAuthor: 'test',
            sourceName: 'test',
        ...
        // Write B: create a MemoryVersion for the new item — succeeds
        await tx.memoryVersion.create({
          data: {
            itemId: newItem.id,
            versionNumber: 1,
            content: 'Write A version',
            snapshot: {},
          },
        });

        // Write C: create a MemoryVersion for the reference item — FAILS
        // version 1 already exists (unique constraint on @@unique([itemId, versionNumber]))
        await tx.memoryVersion.create({
          data: {
            itemId: refItem.id,
            versionNumber: 1,
            content: 'Duplicate version for reference item',
            snapshot: {},
          },
        });
    ...
    // Step 3: Verify — Write A did NOT persist (transaction rolled back)
    const orphanedItems = await prisma.memoryItem.findMany({
      where: { content: 'Write A — should not persist' },
    });
    expect(orphanedItems).toHaveLength(0);

    // Step 4: Verify — Write B did NOT persist (transaction rolled back)
    const orphanedVersions = await prisma.memoryVersion.findMany({
      where: { content: 'Write A version' },
    });
    expect(orphanedVersions).toHaveLength(0);

    // Step 5: Verify — reference item unchanged
    const verifyRef = await prisma.memoryItem.findUnique({
      where: { id: refItem.id },
    });
    expect(verifyRef).not.toBeNull();
    expect(verifyRef!.content).toBe('Reference item for C4/C5 atomicity');
  });
});
