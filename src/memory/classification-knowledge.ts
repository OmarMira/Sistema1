...
export type ConfidenceEvolutionReason =
  | 'human_confirmation'
  | 'deterministic_conflict'
  | 'human_rehabilitation';
...
// ─── Human Rehabilitation (KE-EVOL-005) ─────────────────────────
//
// Explicit HUMAN rehabilitation of knowledge that was degraded by a
// persisted deterministic conflict and whose enabling conflict has since
// been EXPLICITLY RESOLVED by a human.
//
// Immutable rules:
//   AUTOMATIC_REHABILITATION = FORBIDDEN (no thresholds, counts, time
//   windows, scores, decay, or probabilities).
//   REHABILITATION_ACTOR = HUMAN_EXPLICIT (rehabilitatedBy is mandatory).
//   REHABILITATION_TARGET_CONFIDENCE = 'certain'.
//   REHABILITATION_REASON = 'human_rehabilitation'.
//   CONFLICT_RESOLUTION_REQUIRED = YES.
//   ALL_PENDING_CONFLICTS_FOR_ITEM_MUST_BE_CLEAR = YES.
//   FORWARD_CONFLICT_LINKAGE_IS_AUTHORITY = YES (the implicated item ids
//   travel INSIDE the persisted conflict content — no reverse linkage is
//   required, no inference by GL/entityId/description/timestamp/logs).
//   ORPHAN_UNCERTAIN_REHABILITATION = FORBIDDEN (uncertain → certain only
//   with a demonstrable link to a persisted, resolved conflict).
//   LEGACY exact targets are NOT guessed: an AUTHORIZED_VS_EXACT conflict
//   without exactTreatmentItemIds leaves the exact item non-derivable.
//
// Storage decision for traceability (§6): ConfidenceLog + TraceabilityLog
// capture the confidence transition and its reason but do NOT carry the
// enabling conflictItemId, and the resolution record links conflict →
// resolution only. To persist the knowledge item → enabling conflict link
// durably and honestly, the rehabilitation event is stored as a SEPARATE
// MemoryItem of type 'classification_rehabilitation_event' — the same
// append-only type-constant pattern used by
// 'classification_conflict_resolution' (KE-EVOL-004). This is a type
// constant, NOT a schema change.

export const REHABILITATION_EVENT_TYPE = 'classification_rehabilitation_event';

/** Persisted rehabilitation event content (MemoryItem.content JSON, fixed field order). */
export interface RehabilitationEventContent {
  companyId: string;
  /** MemoryItem id of the rehabilitated knowledge item (pattern or exact treatment) */
  knowledgeItemId: string;
  /** Real MemoryItem id of the resolved conflict that enables the rehabilitation */
  conflictItemId: string;
  /** Non-empty human identity who rehabilitated the knowledge */
  rehabilitatedBy: string;
  /** ISO-8601 timestamp generated by the system at rehabilitation time */
  rehabilitatedAt: string;
}

function isValidRehabilitationEventContent(content: unknown): content is RehabilitationEventContent {
  if (!isRecord(content)) return false;
  const nonEmptyString = (value: unknown): boolean => typeof value === 'string' && value !== '';
  return (
    nonEmptyString(content.companyId)
    && nonEmptyString(content.knowledgeItemId)
    && nonEmptyString(content.conflictItemId)
    && nonEmptyString(content.rehabilitatedBy)
    && nonEmptyString(content.rehabilitatedAt)
  );
}

/**
 * Tenant-scoped set of conflict MemoryItem ids that have a valid persisted
 * resolution record. Throws on adapter failure — callers must surface
 * ERROR, never silently treat an unknown state as resolved or pending.
 */
async function getResolvedConflictItemIds(adapter: MemoryAdapter, companyId: string): Promise<Set<string>> {
  const resolutionItems = await adapter.getByType(companyId, CONFLICT_RESOLUTION_TYPE);
  ...
  for (const item of resolutionItems) {
    ...
    try {
      const parsed: unknown = JSON.parse(item.content);
      if (
        isValidConflictResolutionContent(parsed)
        && parsed.companyId === companyId
      ) {
        resolved.add(parsed.conflictItemId);
      }
    } catch {
      // Malformed resolution content — cannot prove resolution; skip
    }
...
export type ConflictResolvedCheck = { resolved: boolean };

/**
 * Read-only check: does the given conflict MemoryItem already have a
 * valid persisted resolution record within the tenant?
 *
 * Throws (propagates adapter errors) so call sites never guess on an
 * unknown state — a read failure is an explicit error, not "not resolved".
 */
export async function isConflictResolved(
  adapter: MemoryAdapter,
  companyId: string,
  conflictItemId: string,
): Promise<ConflictResolvedCheck> {
  if (!companyId || typeof companyId !== 'string') {
    throw new Error('Invalid companyId');
  }
  if (!conflictItemId || typeof conflictItemId !== 'string') {
    throw new Error('Invalid conflictItemId');
  }
  const resolved = await getResolvedConflictItemIds(adapter, companyId);
  return { resolved: resolved.has(conflictItemId) };
}

export type PendingConflictImplicationCheck = { implicated: boolean };

/**
 * Read-only check: is this knowledge item implicated by ANY pending
 * (unresolved) classification_conflicting_pattern, company-wide?
 *
 * Implication is the FORWARD linkage written into the conflict content
 * itself: membership in authorizedPatternIds, or — for AUTHORIZED_VS_EXACT
 * conflicts — membership in exactTreatmentItemIds (legacy conflicts that
 * omit the field cannot implicate an exact item and are not guessed).
 * No classification/type-driven lookup, no entityId inference.
 *
 * Throws on adapter failure so callers never promote/degrade on unknown.
 */
export async function isKnowledgeImplicatedByPendingConflict(
  adapter: MemoryAdapter,
  companyId: string,
  knowledgeItemId: string,
): Promise<PendingConflictImplicationCheck> {
  if (!companyId || typeof companyId !== 'string') {
    throw new Error('Invalid companyId');
  }
  if (!knowledgeItemId || typeof knowledgeItemId !== 'string') {
    throw new Error('Invalid knowledgeItemId');
  }

  const resolvedIds = await getResolvedConflictItemIds(adapter, companyId);
  const items = await adapter.getByType(companyId, CONFLICTING_PATTERN_TYPE);
  for (const item of items) {
    if (item.status !== 'active') continue;
    ...
    try {
      const parsed: unknown = JSON.parse(item.content);
      if (!isValidConflictingPatternContent(parsed)) continue;
      if (parsed.authorizedPatternIds.includes(knowledgeItemId)) {
        return { implicated: true };
      }
      if (
        parsed.kind === 'AUTHORIZED_VS_EXACT'
        && Array.isArray(parsed.exactTreatmentItemIds)
        && parsed.exactTreatmentItemIds.includes(knowledgeItemId)
      ) {
        return { implicated: true };
      }
    } catch {
      // Malformed content — cannot prove implication; skip
    }
  }
  return { implicated: false };
}

export type RehabilitationResult =
  | { status: 'REHABILITATED'; knowledgeItemId: string; conflictItemId: string; rehabilitationEventId: string }
  | { status: 'ALREADY_CERTAIN'; knowledgeItemId: string }
  | { status: 'CONFLICT_NOT_RESOLVED'; conflictItemId: string }
  | { status: 'OTHER_PENDING_CONFLICT'; conflictItemId: string; blockingConflictItemIds: string[] }
  | { status: 'NOT_IMPLICATED'; conflictItemId: string }
  | { status: 'NOT_UNCERTAIN'; knowledgeItemId: string }
  | { status: 'NOT_FOUND' }
  | { status: 'ERROR'; error: string };

/**
 * Explicit human rehabilitation of knowledge degraded by a persisted,
 * EXPLICITLY RESOLVED deterministic conflict.
 *
 * Preconditions, in order:
 *   A. Tenant: conflict AND knowledge item must belong to companyId
 *      (tenant-safe getById — no existence leak, foreign/missing = NOT_FOUND).
 *   B. Conflict type must be classification_conflicting_pattern
 *      (wrong type → NOT_FOUND, same precedent as resolveClassificationConflict).
 *   C. The conflict must have a valid persisted resolution record,
 *      else CONFLICT_NOT_RESOLVED.
 *   D. Explicit implication: knowledgeItemId must appear in the conflict's
 *      own authorizedPatternIds, or — for AUTHORIZED_VS_EXACT — in its
 *      exactTreatmentItemIds. NO inference by GL/entityId/description/
 *      timestamps/ConfidenceLog → NOT_IMPLICATED.
 *   E. Legacy AUTHORIZED_VS_EXACT conflicts (no exactTreatmentItemIds):
 *      only the explicitly identified pattern may proceed; the exact item
 *      is NOT derivable → NOT_IMPLICATED (never guessed).
 *   F. Current state: knowledge item confidence must be 'uncertain'
 *      ('certain' → ALREADY_CERTAIN; any other state → NOT_UNCERTAIN).
 *   G. Multi-conflict safety: a company-wide scan of remaining pending
 *      conflicts (resolution records excluded). ANY other pending conflict
 *      implicating the knowledge item → OTHER_PENDING_CONFLICT and
 *      NOTHING is modified.
 *
 * On success: confidence evolves to 'certain' via the EXISTING C11 path
 * (evolveClassificationConfidence → ConfidenceLog + TraceabilityLog,
 * reason 'human_rehabilitation') — no direct DB confidence writes, no
 * second confidence system — plus one append-only rehabilitation event
 * MemoryItem preserving the enabling conflict linkage.
 *
 * Chooses no winner, mutates no treatment/pattern content, no GL accounts.
 */
export async function rehabilitateClassificationKnowledge(
  adapter: MemoryAdapter,
  companyId: string,
  conflictItemId: string,
  knowledgeItemId: string,
  ...
  const invalidInput = !companyId || typeof companyId !== 'string'
    || !conflictItemId || typeof conflictItemId !== 'string'
    || !knowledgeItemId || typeof knowledgeItemId !== 'string'
    || !rehabilitatedBy || typeof rehabilitatedBy !== 'string' || rehabilitatedBy.trim() === '';
  if (invalidInput) {
    return { status: 'ERROR', error: 'companyId, conflictItemId, knowledgeItemId and rehabilitatedBy are required' };
  }

  try {
    // A + B: tenant-safe conflict retrieval, exact type required
    const conflictItem = await adapter.getById(conflictItemId, companyId);
    if (!conflictItem || conflictItem.type !== CONFLICTING_PATTERN_TYPE) {
      return { status: 'NOT_FOUND' };
    }
    ...
    // A: tenant-safe knowledge retrieval (foreign/missing → NOT_FOUND)
    const knowledgeItem = await adapter.getById(knowledgeItemId, companyId);
    if (!knowledgeItem) {
      return { status: 'NOT_FOUND' };
    }

    // C: the enabling conflict must be explicitly resolved by a human
    const resolvedConflicts = await getResolvedConflictItemIds(adapter, companyId);
    if (!resolvedConflicts.has(conflictItemId)) {
      return { status: 'CONFLICT_NOT_RESOLVED', conflictItemId };
    }

    // D + E: explicit FORWARD implication only; legacy exact targets
    // without exactTreatmentItemIds are NOT derivable and never guessed.
    const implicatesKnowledge =
      conflictContent.authorizedPatternIds.includes(knowledgeItemId)
      || (
        conflictContent.kind === 'AUTHORIZED_VS_EXACT'
        && Array.isArray(conflictContent.exactTreatmentItemIds)
        && conflictContent.exactTreatmentItemIds.includes(knowledgeItemId)
      );
    if (!implicatesKnowledge) {
      return { status: 'NOT_IMPLICATED', conflictItemId };
    }

    // F: current state must be 'uncertain'
    const currentConfidence = knowledgeItem.confidence;
    if (currentConfidence === 'certain') {
      return { status: 'ALREADY_CERTAIN', knowledgeItemId };
    }
    if (currentConfidence !== 'uncertain') {
      return { status: 'NOT_UNCERTAIN', knowledgeItemId };
    }

    // G: company-wide pending-conflict safety. The rehabilitation-enabling
    // conflict is resolved, so it cannot appear here. Any OTHER pending
    // conflict implicating the knowledge item blocks the rehabilitation.
    const pendingItems = await adapter.getByType(companyId, CONFLICTING_PATTERN_TYPE);
    const blockingConflictItemIds: string[] = [];
    for (const item of pendingItems) {
      if (item.status !== 'active') continue;
      if (resolvedConflicts.has(item.id)) continue;
      try {
        const parsed: unknown = JSON.parse(item.content);
        if (!isValidConflictingPatternContent(parsed)) continue;
        const implicatesPending =
          parsed.authorizedPatternIds.includes(knowledgeItemId)
          || (
            parsed.kind === 'AUTHORIZED_VS_EXACT'
            && Array.isArray(parsed.exactTreatmentItemIds)
            && parsed.exactTreatmentItemIds.includes(knowledgeItemId)
          );
        if (implicatesPending) {
          blockingConflictItemIds.push(item.id);
        }
      } catch {
        // Malformed content — cannot prove implication; skip
      }
    }
    if (blockingConflictItemIds.length > 0) {
      return {
        status: 'OTHER_PENDING_CONFLICT',
        conflictItemId,
        blockingConflictItemIds,
      };
    }

    // Confidence transition through the EXISTING C11 infrastructure.
    const evolved = await evolveClassificationConfidence(
      adapter,
      companyId,
      knowledgeItemId,
    ...
    if (evolved.status === 'NOT_FOUND') {
      // Lost between the precheck and the write — surface honestly.
      return { status: 'NOT_FOUND' };
    }
    if (evolved.status === 'ERROR') {
      return { status: 'ERROR', error: evolved.error };
    }
    if (evolved.status === 'UNCHANGED') {
      // Confidence already 'certain' between the precheck and the write —
      // idempotent rehabilitation end-state.
      return { status: 'ALREADY_CERTAIN', knowledgeItemId };
    }

    // Traceability linkage (§6): append-only rehabilitation event MemoryItem
    // preserving knowledgeItemId → conflictItemId → resolution provenance.
    const eventContent: RehabilitationEventContent = {
      companyId,
      knowledgeItemId,
      conflictItemId,
      rehabilitatedBy,
      rehabilitatedAt: new Date().toISOString(),
    };
    const event = await adapter.record({
      content: JSON.stringify(eventContent),
      type: REHABILITATION_EVENT_TYPE,
      companyId,
      sourceAuthor: rehabilitatedBy,
      sourceName: 'classification_rehabilitation',
      sourceObservedAt: new Date(),
      confidence: 'certain',
    });
    ...
    return {
      ...
      knowledgeItemId,
      conflictItemId,
      rehabilitationEventId: event.id,
    };
  } catch (error) {
    return {
      status: 'ERROR',
